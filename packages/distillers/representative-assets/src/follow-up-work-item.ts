...
const factory: DistillerFactory = () =>
	createRepresentativeDistiller<FollowUpWorkItemPayload>({
		...
		parsePayload(item) {
			...
			if (rawPriority !== undefined && priorityHint === undefined) return undefined;
			return {
				action,
				reason,
		...
		tags(payload) {
			return payload.priority_hint ? ["follow-up-work-item", payload.priority_hint] : ["follow-up-work-item"];
		},
	});

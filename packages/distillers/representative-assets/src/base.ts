import type { DistillResultDraft, DistillerPlugin } from "@loamlog/core";
...
interface RepresentativeDistillerSpec<TPayload extends Record<string, unknown>> {
	id: string;
	name: string;
	version: string;
	type: string;
...
export function createRepresentativeDistiller<TPayload extends Record<string, unknown>>(
	spec: RepresentativeDistillerSpec<TPayload>,
): DistillerPlugin {

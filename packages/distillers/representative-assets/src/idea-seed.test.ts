...
describe("idea-seed distiller", () => {
	test("returns idea seed assets with valid evidence", async () => {
		const outputs = await factory().run(makeRunInput([
			{
				...
				context: "The user said AI sessions contain ideas that are lost while pushing current work forward.",
				next_probe: "Try extracting idea seeds from three recent sessions.",
				confidence: 0.86,
				evidence_refs: [{ message_id: "msg_1", excerpt: "很多想法或者经验要沉淀" }],
			},
		]));
		...
		assert.equal(outputs[0].payload.idea, "Capture ideas before they are forgotten");
		assert.equal(outputs[0].payload.next_probe, "Try extracting idea seeds from three recent sessions.");

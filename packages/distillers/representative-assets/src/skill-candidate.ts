...
const PROMOTION_TARGETS = ["codex_skill", "agents_rule", "prompt_template", "runbook", "project_doc"] as const;
...
const factory: DistillerFactory = () =>
	createRepresentativeDistiller<SkillCandidatePayload>({
		...
		parsePayload(item) {
			...
			const promotionTarget = getEnumValue(item, "promotion_target", PROMOTION_TARGETS);
			if (!skillName || !trigger || !capability || !workflowSteps) return undefined;
			if (rawPromotionTarget !== undefined && promotionTarget === undefined) return undefined;
			return {
		...
		tags(payload) {
			return payload.promotion_target ? ["skill-candidate", payload.promotion_target] : ["skill-candidate"];
		},
	});

...
describe("follow-up-work-item distiller", () => {
      ...
      makeRunInput([
        {
          ...
          reason:
            "The design has been approved and needs a runnable vertical slice.",
          priority_hint: "p0",
  ...
  test("drops follow-up items when evidence does not show open work", async () => {
    const outputs = await factory().run(
      makeRunInput([
        {
          action: "Refine provider prompt and error handling",
          reason: "The session mentioned a provider prompt risk.",
          acceptance: ["Provider prompt behavior is reviewed"],
          confidence: 0.8,
          evidence_refs: [
            { message_id: "msg_2", excerpt: "provider prompt risk" },
          ],
        },
      ]),
    );
  ...
  return {
    ...
    messages: [
      ...
      },
      {
        ...
        role: "user",
        timestamp: "2026-05-13T00:00:00.000Z",
        content: "Provider prompt risk belongs in a practice or pitfall note.",
      },
    ],

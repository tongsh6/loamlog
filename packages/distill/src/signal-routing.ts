...
export function selectSignalsForDistiller(
	...
	for (const signal of signals) {
		if (options.sessionId && !signal.spans.some((span) => span.session_id === options.sessionId)) {
			continue;
		}
...
function hasIneligiblePromotionHint(signal: Signal, distillerId: string): boolean {
	return signal.promotion_hints.some(
		(hint) =>
			hint.target_distiller === distillerId && hint.eligibility === "ineligible",
	);
}
...
function matchSignalRule(
	...
	if (rule.min_confidence !== undefined && effective.confidence < rule.min_confidence) {
		return undefined;
	}
	...
	if (
		rule.allowed_actors &&
		!rule.allowed_actors.includes(effective.actor)
	) {
		return undefined;
	}
	if (

...
- Signal Gate 的 contract / store / 最小人工 CLI / classifier schema / DAG 内 signal-based routing 已落成：`SignalKind`、`SignalTag`、`LocalAssetStore` signal 节点、`loam signal list/show/review`、`signal-classifier` schema/prompt/normalize helper、manifest-based routing 与 `SignalConsumption` lineage 已可用于下一步 distiller repair；capture 后自动 Signal Gate job 与 classifier rerun 命令仍待实现。
- The Signal Gate contract / store / minimal human CLI / classifier schema / in-DAG signal-based routing are implemented: `SignalKind`, `SignalTag`, `LocalAssetStore` signal nodes, `loam signal list/show/review`, `signal-classifier` schema/prompt/normalize helpers, manifest-based routing, and `SignalConsumption` lineage are ready for distiller repair; automatic post-capture Signal Gate jobs and classifier rerun commands remain.
- 代表性资产通用过滤层 v0.1 已落成：共享 post-filter 已覆盖 assistant process log、动作壳标题、follow-up 缺验收标准、done-state / old-roadmap idea、非明确 decision、普通 API key 错误经验、one-off command / bug fix / CI skill-candidate；仍需 duplicate-topic 与真实样本 prompt/schema repair。
- Representative asset common post-filter v0.1 is implemented: the shared post-filter covers assistant process logs, action-shell titles, follow-up items without acceptance criteria, done-state / old-roadmap ideas, non-explicit decisions, plain API-key error “pitfalls”, and one-off command / bug-fix / CI skill candidates; duplicate-topic handling and real-sample prompt/schema repair remain.

...
- 代表性资产通用过滤层 v0.3 与 prompt/schema repair v0.4 已落成：共享 post-filter 已覆盖 assistant process log、动作壳标题、follow-up 缺验收标准、done-state / old-roadmap idea、非明确 decision、普通 API key 错误经验、one-off command / bug fix / CI skill-candidate，并新增 API key 排障不进 idea、routine repo implementation / dogfooding execution 不进 idea、Loamlog 内部 dogfooding / provider bug / CI 构建动作不升为 skill-candidate；5 类 distiller 也已复用 evidence-first prompt guardrails 和类型专属负例，减少无证据扩写与 wrong-type 输出；仍需跨候选 duplicate-topic 合并与小批量复评。
- Representative asset common post-filter v0.3 and prompt/schema repair v0.4 are implemented: the shared post-filter covers assistant process logs, action-shell titles, follow-up items without acceptance criteria, done-state / old-roadmap ideas, non-explicit decisions, plain API-key error “pitfalls”, and one-off command / bug-fix / CI skill candidates, and now rejects low-value duplicate topics such as API-key troubleshooting as ideas, routine repo implementation / dogfooding execution as ideas, and Loamlog-internal dogfooding / provider-bug / CI-build work as skill candidates; all five distillers now share evidence-first prompt guardrails and type-specific negative examples to reduce unsupported expansion and wrong-type output; cross-candidate duplicate-topic merging and small-batch re-review remain.

...
- 代表性资产通用过滤层 v0.2 已落成：共享 post-filter 已覆盖 assistant process log、动作壳标题、follow-up 缺验收标准、done-state / old-roadmap idea、非明确 decision、普通 API key 错误经验、one-off command / bug fix / CI skill-candidate，并新增 API key 排障不进 idea、Loamlog 内部 dogfooding / provider bug / CI 构建动作不升为 skill-candidate 的低价值重复主题过滤；仍需跨候选 duplicate-topic 合并与真实样本 prompt/schema repair。
- Representative asset common post-filter v0.2 is implemented: the shared post-filter covers assistant process logs, action-shell titles, follow-up items without acceptance criteria, done-state / old-roadmap ideas, non-explicit decisions, plain API-key error “pitfalls”, and one-off command / bug-fix / CI skill candidates, and now rejects low-value duplicate topics such as API-key troubleshooting as ideas and Loamlog-internal dogfooding / provider-bug / CI-build work as skill candidates; cross-candidate duplicate-topic merging and real-sample prompt/schema repair remain.
